        ...
        for fil in range(net.filter_count):
            ...
            for i in range(net.filter_size):
                for j in range(net.filter_size):
                    ...
                    net.update_fil[conv][fil][i][j] =  sum(prv[max(0, i-net.step):min(w, w+i-net.step),max(0, j-net.step):min(h, h+j-net.step)]
                                                            *conv_delta[conv][fil][max(0, net.step-i):min(w, w+net.step-i),max(0, net.step-j):min(h, h+net.step-j)])                          
            
            filters[conv][fil]   += net.learn_rate_conv*net.update_fil[conv][fil]
            conv_bias[conv][fil] += net.learn_rate_conv*sum(conv_delta[conv][fil])
